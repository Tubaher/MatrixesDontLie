...
#define N_SAL   22         // 11 neuronas por eje, NEURONAS SALIDAS 
#define N_HID   100        // 19      //NEURONAS ESCONlDIDAS MAXIMAS
...
//=======================================================================================================================
float  Mid=0.5, Eta=0.025, Gain=1.5;     //0.50;  //Eta anterior 0.25
int  four,xtal, five;
float Target[N_SAL];
float Winner[N_SAL];
...
void limpiar_pesos(void)
{
  ...
  // Limpiar de pesos para la capa escondida
    for (k=0; k<N_HID ;k++)
    {
        ...
        for (i=0; i<N_IN; i++)
        {
          c_escondida.pesos[k][i]=peso_aleatorio()*0.001;    //c_escondida.pesos[k][i]=0;
          //pump_xtal();    
        }    
    }    
