...
int CargarPesos(void)
{
	...
	FILE *in_file;
	aux=0;
	if ((in_file = fopen ("hidden_weights.dat", "rt")) == NULL)
		aux=1;
	else
	{
	...
	fclose(in_file);
	if ((in_file = fopen ("out_weights.dat", "rt")) == NULL)
		aux=aux+2;
	else
	{
		fseek(in_file, 0, SEEK_SET);
...
int SalvarPesos(void)
{
	...
	FILE *out_file;
	aux=0;
	if ((out_file = fopen ("hidden_weights.dat", "wt")) == NULL)
		aux=1;
	else
	{
	...
	if ((out_file = fopen ("out_weights.dat", "wt")) == NULL)
		aux=aux+2;
	else
	{
		fseek(out_file, 0, SEEK_SET);
